...
def parse_directory_listing(html):
    ...
    if table_entries:
        for row in table_entries:
            ...
            # Check if it's a directory based on the icon
            is_dir = bool(row.find('img', alt='[DIR]') or 
                         row.find('img', src='/icons/folder.gif') or
                         row.find('img', alt='[PARENTDIR]'))
    ...
    # Try unordered list format
    else:
        list_entries = soup.find_all('li')
        if list_entries:
            for item in list_entries:
                ...
                # For list format, assume it's a directory if the href ends with /
    ...
    if not entries:
        print("Warning: No entries found in directory listing!")
        print("HTML content:")
        print(html)
